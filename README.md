...
- 결과 화면에 **YouTube ↗** 바로가기 링크, 타임스탬프 켜면 **각 타임스탬프 클릭 시 유튜브 해당 지점으로 점프** — 영상↔텍스트 오가며 보기
- 배치 최대 **50개**로 확대
- 자막 요청 **자동 페이싱** — 한 번에 하나씩, 요청 사이 2~5초 랜덤 간격 (봇 감지 회피)
- **차단 자동 복구** — 429 감지 시 서버가 전체 쿨다운(90초부터, 반복 시 최대 15분까지 지수 증가)을 걸고, 화면에서 카운트다운 후 자동 재시도 (최대 5회)
- 긴 영상(1.5시간↑)은 1시간 단위로 잘라 받아쓰기 — 초장편 Whisper 중 메모리 부족으로 서버가 죽던 문제 해결
- 재시도 **[중지]** 버튼 (행별) + **[차단 재시도 전체 중지]** 버튼
- GPU 라이브러리(cuBLAS/cuDNN)를 코드가 직접 로드 — `libcublas.so.12 not found` 방지, GPU 로드 실패 시 CPU 자동 폴백

## 버전 히스토리

| 버전 | 내용 |
|------|------|
| v4.2 | 유튜브 바로가기·타임스탬프 점프, 배치 50개, 자막 페이싱·차단 자동 복구, 긴 영상 청크 분할 |
